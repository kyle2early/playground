...
pixar_all.plot(kind='scatter',x='Release Yr',y='Rotten Tomatoes',s=pixar_all['NA Gross']*2,grid=True,color=pixar_all['NA Gross'],cmap='RdYlBu_r')
...
# Identify lowest reviewed
pixar_rotten = pixar_all.sort_values(by=['Rotten Tomatoes'], ascending=False).tail(1)
for index, row in pixar_rotten.iterrows():
    plt.annotate(row['Film'],xy=(row['Release Yr'],row['Rotten Tomatoes']),xytext=(-25,-25),textcoords='offset points',arrowprops=dict(arrowstyle='->',connectionstyle='arc3,rad=0'),bbox=dict(facecolor='red',boxstyle='round,pad=0.2'))


# Save plot
plt.savefig("pixar_NA_Earnings_vs_Reviews.png")
# show plot
...
plt.legend(['[MM USD]'])
plt.savefig("pixar_NA_Earnings_histogram.png")
plt.show()
